...
const integrationFallbacks = {
  ...
  yjs: "@automerge/automerge and append-only JSON records remain available",
  "@automerge/automerge": "Yjs and append-only JSON records remain available",
  "isomorphic-git": "native git bridge remains available",
  "simple-git": "native git bridge remains available",
  "@lancedb/lancedb":
    "SQLite-vec/Qdrant-compatible adapter slot and keyword search remain available",
  "sqlite-vec": "LanceDB and keyword search remain available",
  "@huggingface/transformers":
    "Ollama/llama.cpp/vLLM/SGLang adapters remain available",
  "onnxruntime-node":
    "@huggingface/transformers and local model server adapters remain available",
  ...
  typescript:
    "plain JavaScript fallback remains available for emergency patches",
  "@capacitor/core":
    "bundled web UI remains available in a browser/PWA fallback",
  "@capacitor/android":
    "bundled web UI remains available in a browser/PWA fallback",
  "@capacitor/cli":
    "manual Android Studio project setup remains available after web build",
  "@capacitor/app": "browser/PWA lifecycle fallback remains available",
  "@capacitor/device": "manual device labels remain available",
  "@capacitor/filesystem":
    "Capacitor Preferences and local backend records remain available",
  "@capacitor/network": "API health checks remain available",
  "@capacitor/preferences":
    "localStorage and backend settings records remain available",
  ...
  for (const group of [
    ...
    for (const [name, version] of Object.entries(pkg[group] || {})) {
      inventory.push({
        ...
        version,
        fallback:
          integrationFallbacks[name] || "No specific fallback registered yet",
      });
    }
  }
}
...
fs.writeFileSync(
  path.join(root, "dist/aift-forge-dependencies.json"),
  ...
  JSON.stringify(
    {
      ...
      output: "dist/aift-forge-dependencies.json",
    },
    null,
    2,
  ),
