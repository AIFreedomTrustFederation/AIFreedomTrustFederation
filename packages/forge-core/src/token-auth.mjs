...
import { makeId, readState, writeState } from "./store.mjs";
...
export function authenticateToken(input = {}) {
  ...
  const record = (state.tokens || []).find(
    (item) => item.token_hash === tokenHash && item.status === "active",
  );
  if (!record)
    return {
      authenticated: false,
      actor: { user_id: "anonymous", role: "guest" },
      scopes: [],
    };
  return {
    authenticated: true,
...
export function revokeToken(input = {}) {
  ...
  const tokens = (state.tokens || []).map((record) =>
    record.token_id === tokenId
      ? { ...record, status: "revoked", revoked_at: now() }
      : record,
  );
